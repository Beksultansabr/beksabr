  ...
  if (items) {
    ...
    items.forEach((itemBlock) => {
      ...
      let master = gsap
        ...
        .add(marquee(marqueeObject.el,90, dirFromRight), 0);
...
// Найти видео элементы
const introVideo = document.querySelector('.intro-video');
const mainVideo = document.querySelector('.main-video');

// Добавить событие по завершению цикла intro-video
introVideo.addEventListener('ended', () => {
  introVideo.style.opacity = 0; // Убираем приветственное видео
  mainVideo.style.opacity = 1; // Показываем основное видео
  mainVideo.play(); // Запускаем основное видео
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (playButton && video) {
    ...
    console.error("Кнопка или видео не найдены!");
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  playButton.addEventListener("mousemove", (e) => {
    ...
    const x = ((e.clientX - rect.left) / rect.width - 0.5) * 20; // Движение по оси X
    const y = ((e.clientY - rect.top) / rect.height - 0.5) * 20; // Движение по оси Y
    playButton.style.transform = `translate(${x}px, ${y}px)`; // Применение трансформации
  });

  playButton.addEventListener("mouseleave", () => {
    playButton.style.transform = "translate(0, 0)"; // Возврат на исходное место
  });
});
